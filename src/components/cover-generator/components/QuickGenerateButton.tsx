...
import type { QuickModelOption } from "../types"

interface QuickGenerateButtonProps {
  isGenerating: boolean
  title: string
  genre: string
  selectedModelConfig: QuickModelOption
  onGenerate: () => void
}

export function QuickGenerateButton({
  isGenerating, title, genre, selectedModelConfig, onGenerate,
}: QuickGenerateButtonProps) {
  return (
    <section>
      <button
        onClick={onGenerate}
        disabled={isGenerating || !title || !genre || !selectedModelConfig.available}
      ...
      >
        {isGenerating ? (
          ...
          <>
            ...
            一键生成封面 · {selectedModelConfig.credits}点
          </>
        )}
      </button>
